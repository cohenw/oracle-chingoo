...
<%
	...
	String cnt= cn1.queryOne(sql, false);
	if (cnt.equals("0")) {
		out.println("MKEY ["+mkey+"] not exists in DB1.");
		return;
	}
	...
	cnt= cn2.queryOne(sql, false);
	if (!cnt.equals("0")) {
		out.println("MKEY ["+newmkey+"] exists in DB2.");
		return;
	}
...
//	if (1>0) return;
    ...
    try {
    	...
    	oStmt = oCon1.prepareCall(cStmt);
		oStmt.setString(1,cn1.getSchemaName().toUpperCase());
...
//        oCon.commit();
	...
	cn2.getConnection().setAutoCommit(false);
...
//		Util.p("$$$$$");
		try {
			CallableStatement call = cn2.getConnection().prepareCall(stmt);
			call.execute();
			call.close();
   	 	} catch (SQLException e) {
      	  	e.printStackTrace();
      	  	...
      	  	out.println(e);
      	  	error = true;
      	  	cn2.getConnection().rollback();
      	 	cn2.getConnection().setAutoCommit(true);
      	  	break;
    	}
	}
