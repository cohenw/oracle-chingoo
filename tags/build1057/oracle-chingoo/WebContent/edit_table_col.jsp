...
<%
	...
	if (submit != null) {
		...
		try {
			...
			String sql = "DELETE FROM CHINGOO_TABLE_COL WHERE TNAME='" + Util.escapeQuote(table) + "'";
			...
			for (int i=0;i<list.size();i++) {
				...
				if (caption.length() > 0 || lookupSql.length() > 0) {
					sql = "INSERT INTO CHINGOO_TABLE_COL (TNAME, CNAME, CAPT, LOOKUP_SQL) VALUES (?,?,?,?)";
...
<%	
	...
	for (int i=0;i<list.size();i++) {
		...
		String caption = cn.queryOne("SELECT CAPT FROM CHINGOO_TABLE_COL WHERE TNAME='" + table +"' AND CNAME='" + col_name +"'", false);
		String lookupSql = cn.queryOne("SELECT LOOKUP_SQL FROM CHINGOO_TABLE_COL WHERE TNAME='" + table +"' AND CNAME='" + col_name +"'", false);
//System.out.println("SELECT CAPT FROM CHINGOO_TABLE_COL WHERE TNAME='" + table +"' AND CNAME='" + col_name +"'");
