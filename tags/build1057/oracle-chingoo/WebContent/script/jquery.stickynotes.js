...
*/		}
	...
	jQuery.fn.stickyNotes.createNote2 = function(noteText, posLeft, posTop, posWidth, posHeight) {
		var pos_x = posTop;
		var pos_y = posLeft;		
		var note_id = jQuery.fn.stickyNotes.notes.length + 1;
		var _note_content = jQuery(document.createElement('textarea')).append(noteText);
		...
		var _div_delete = 	$(document.createElement('div'))
							...
							.click(function(){jQuery.fn.stickyNotes.deleteNote(this);});
		var _div_wrap 	= 	$(document.createElement('div'))
							.css({'position':'absolute','top':pos_x,'left':pos_y, 'width':posWidth,'height':posHeight,'float' : 'left'})
							.attr("id", "note-" + note_id)
							.append(_div_background)
							.append(_div_note)
							.append(_div_delete);	
    ...
    jQuery.fn.stickyNotes.createNoteBackground = function() {
        ...
        if (jQuery.browser.msie && jQuery.browser.version <= 6)  {
            background = $(document.createElement('div')).addClass("background").html('<img src="image/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'image/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
        } else {
            background = $(document.createElement('div')).addClass("background").html('<img src="image/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
        }
        return background;
