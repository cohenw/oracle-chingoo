...
<%
	...
	String q2 = "SELECT object_name FROM user_objects A where object_type='TRIGGER' AND object_name IN (SELECT NAME FROM USER_DEPENDENCIES WHERE REFERENCED_NAME='" + table + "' AND TYPE in ('TRIGGER')) AND NOT EXISTS (SELECT 1 FROM CHINGOO_TR WHERE TRIGGER_NAME=A.OBJECT_NAME AND CREATED > A.LAST_DDL_TIME) ORDER BY 1";
	...
	String qry = "SELECT PACKAGE_NAME, PROCEDURE_NAME, COLS_INSERT, COLS_UPDATE, COLS_DELETE FROM CHINGOO_PA_TABLE WHERE TABLE_NAME='" +  tname + "' order by 1, 2";
	List<String[]> ql = cn.query(qry, false);

	String qry2 = "SELECT TRIGGER_NAME, COLS_INSERT, COLS_UPDATE, COLS_DELETE FROM CHINGOO_TR_TABLE WHERE TABLE_NAME='" +  tname + "' order by 1";
...
<img src="image/matrix.gif" align="middle"/>
<span style="color: blue; font-family: Arial; font-size:16px; font-weight:bold;">CRUD Matrix</span>
<b><%= cn.getUrlString() %></b>
&nbsp;&nbsp;&nbsp;&nbsp;
...
List<String> refProc = cn.getReferencedProc(tname);
	...
	if (refProc.size() > 0) { 
%>
<img src="image/chingoo-icon.png"> <b>Referenced By</b>
