...
public class TriggerTableWorker {
	...
	public List<String> startWork(Connect cn) throws IOException, SQLException {
		...
		if (cn.isTVS("CHINGOO_TR")) {
			q = "SELECT object_name FROM user_objects A where object_type='TRIGGER' AND NOT EXISTS (SELECT 1 FROM CHINGOO_TR WHERE TRIGGER_NAME=A.OBJECT_NAME AND CREATED > A.LAST_DDL_TIME) ORDER BY 1";
		}
