...
public class CacheManager {
	...
	public CacheSchema getCacheSchema(Connect cn, String databaseUrl, String schemaName) throws SQLException {
		String key = databaseUrl.toLowerCase();
		...
		if (cs==null) {
			...
			// load from database
			cs = new CacheSchema(cn, databaseUrl, schemaName);
			csTable.put(key, cs);
			saveSchemaToFile(cs);
		}
	...
	private void saveSchemaToFile(CacheSchema cs) {
		...
		kryo.register(CacheSchema.class);
   		...
   		try{
   			...
   			Output output = new Output(new FileOutputStream("/home/oracle-chingoo/cache/" + serFilename));
	...
	private CacheSchema readSchemaToFile(String dbUrl) {
		...
		kryo.register(CacheSchema.class);
   		...
   		try{
   			...
   			Input input = new Input(new FileInputStream("/home/oracle-chingoo/cache/" + serFilename));
